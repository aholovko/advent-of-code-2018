...
func TestFindNonOverlappingClaim(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run("1", func(t *testing.T) {
			journal := tc.arg
			gotGuard, gotAsleepAt := journal.findMostAsleepGuard()
			if gotGuard != tc.wantGuard {
				t.Errorf("findMostAsleepGuard(), guard = %d, want %d", gotGuard, tc.wantGuard)
			}
			if gotAsleepAt != tc.wantAsleepAt {
				t.Errorf("findMostAsleepGuard(), asleepAt = %d, want %d", gotAsleepAt, tc.wantAsleepAt)
			}
		})
	}
}
